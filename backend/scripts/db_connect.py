...
Mirrors the logic in spectra-app/src/lib/db.ts:
  - Windows Auth when DB_USER is not set (Trusted_Connection=yes)
  - SQL Auth when DB_USER is set
Env loaded from root .env if it exists, otherwise spectra-app/.env.local.
"""
import os
import pyodbc
...
# Load credentials: prefer root .env, fall back to spectra-app/.env.local
_env_candidates = [
    _PROJECT_ROOT / ".env",
