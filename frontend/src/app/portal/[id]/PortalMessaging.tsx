...
export default function PortalMessaging({ clientId }: Props) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      setMessages(msgs)
      if (!open) {
        setUnread(msgs.filter(m => m.sender_type === 'officer' && !m.read_at).length)
      } else {
        setUnread(0)
      }
    } catch { /* ignore */ }
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes pm-pop { from { opacity:0; transform: scale(0.92) translateY(8px) } to { opacity:1; transform: scale(1) translateY(0) } }
      `}</style>
      ...
      <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
        ...
        {open && (
          <div style={{
            ...
            animation: 'pm-pop 0.18s ease',
          }}>
            {/* Header */}
            <div style={{ background: '#0D1B2A', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 9, flexShrink: 0 }}>
              <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(201,168,76,0.2)', border: '1.5px solid #C9A84C', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M8 2a3 3 0 1 1 0 6 3 3 0 0 1 0-6zM2 13c0-3 2.7-5 6-5s6 2 6 5" stroke="#C9A84C" strokeWidth="1.3" strokeLinecap="round"/></svg>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'white' }}>SPECTRA Bank</div>
                <div style={{ fontSize: 9, color: 'rgba(201,168,76,0.8)' }}>Your Risk Advisor</div>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 12px 4px', display: 'flex', flexDirection: 'column', gap: 8, background: '#F8FAFC' }}>
              {loading ? (
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ width: 20, height: 20, borderRadius: '50%', border: '2.5px solid #E2E8F0', borderTopColor: '#C9A84C', animation: 'pm-spin 0.8s linear infinite' }} />
                </div>
              ) : messages.length === 0 ? (
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8', fontSize: 11, textAlign: 'center', padding: '0 20px' }}>
                  No messages yet.<br />Send a message to your advisor.
                </div>
              ) : messages.map(msg => {
                ...
                if (msg.is_system) {
                  ...
                  const isEngagement = body.includes('📅')
                  ...
                  const needsAction = isDocs || isFreeze || isEngagement
                  return (
                    <div key={msg.id} style={{ width: '100%', background: bgColor, border: `1px solid ${borderColor}40`, borderLeft: `3px solid ${borderColor}`, borderRadius: 8, padding: '10px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: '#0D1B2A' }}>{firstLine}</span>
                        {needsAction && (
                          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 20, background: borderColor, color: 'white', flexShrink: 0, marginLeft: 6 }}>Action Required</span>
                        )}
                      </div>
                      {rest && <div style={{ fontSize: 10, color: '#374151', lineHeight: 1.6, whiteSpace: 'pre-line' }}>{rest}</div>}
                      <div style={{ fontSize: 9, color: '#94A3B8', marginTop: 6 }}>{fmtTime(msg.created_at)} · SPECTRA Bank</div>
                    </div>
                  )
                }
                ...
                return (
                  <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isClient ? 'flex-end' : 'flex-start' }}>
                    <div style={{
                      ...
                      background: isClient ? '#0D1B2A' : 'white',
                      color: isClient ? '#F1F5F9' : '#0D1B2A',
                      boxShadow: '0 1px 2px rgba(0,0,0,0.07)',
                    }}>
                      {msg.body && <div>{msg.body}</div>}
                      {msg.attachment_name && (
                        <a href={msg.attachment_url ?? '#'} download={msg.attachment_name} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: msg.body ? 4 : 0, fontSize: 10, color: isClient ? '#C9A84C' : '#3B82F6', textDecoration: 'none', fontWeight: 600 }}>
                    ...
                    <div style={{ fontSize: 9, color: '#94A3B8', marginTop: 2, display: 'flex', gap: 6 }}>
                      ...
                      {isClient && <span style={{ color: msg.read_at ? '#22C55E' : '#94A3B8' }}>{msg.read_at ? '✓✓' : '✓'}</span>}
                    </div>
                  </div>
                )
              })}
            ...
            <div style={{ padding: '6px 10px 0', background: 'white', flexShrink: 0 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: '#FFFBEB', border: '1.5px dashed #C9A84C', color: '#92400E',
                }}
              >
                <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M13 9l-5 5a4 4 0 0 1-5.66-5.66l6-6a2.5 2.5 0 0 1 3.54 3.54l-6 6a1 1 0 0 1-1.42-1.42l5-5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>
                Send a Document to Your Advisor
              </button>
            </div>

            {/* Input */}
            <div style={{ padding: '6px 10px 8px', display: 'flex', gap: 6, alignItems: 'flex-end', background: 'white', flexShrink: 0 }}>
              <textarea
                value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKeyDown}
                placeholder="Message your advisor..." rows={1}
