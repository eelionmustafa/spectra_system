...
CREATE INDEX IX_DocumentRequests_ClientID_CreatedAt
  ON [SPECTRA].[dbo].[DocumentRequests] (client_id, created_at DESC)
...
export async function markDocumentsReceived(id: string): Promise<void> {
  await ensureTable()
  await query(
    `UPDATE [SPECTRA].[dbo].[DocumentRequests]
